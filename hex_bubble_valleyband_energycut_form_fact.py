...
Iq=[]
for i in range(Npoi):
    kxq,kyq=kwrap_FBZ(KX[i]+qx,KY[i]+qy)
    indmin=np.argmin(np.sqrt((KX-kxq)**2+(KY-kyq)**2))
    KXq.append(kxq)
    KYq.append(kyq)
    Iq.append(indmin)
print("sizes here...", np.shape(KXq), np.shape(Iq) )
VV=np.array(Vertices_list+[Vertices_list[0]])

plt.plot(VV[:,0],VV[:,1])
plt.scatter(KX,KY, c=ff)
plt.show()

plt.plot(VV[:,0],VV[:,1])
# plt.scatter(KX,KY)
plt.scatter(KXq,KYq, c=ff)
plt.show()

plt.plot(VV[:,0],VV[:,1])
plt.scatter(KX[Iq],KY[Iq], c=ff)
plt.show()
...
# thet=-2*np.pi/3
# # thet=np.pi
# Rotth1=np.array([[np.cos(thet),-np.sin(thet)],[np.sin(thet),np.cos(thet)]]) #rotation matrix +thet


# KXrot=Rotth1[0,0]*KX+Rotth1[0,1]*KY
# KYrot=Rotth1[1,0]*KX+Rotth1[1,1]*KY
...
# Lambda_Tens_plus=np.zeros([Npoi,nbands,Npoi,nbands])+0*1j
# Lambda_Tens_min=np.zeros([Npoi,nbands,Npoi,nbands])+0*1j
# for i in range(nbands):
#     for j in range(nbands):
#         for l in range(Npoi):
#             for m in range(Npoi):
#                 Lambda_Tens_plus[l,i,m,j]=Lambda_Tens_plus[l,i,m,j]+np.sum(psi_plus_conj[l,:,:,:,:,i]*psi_plus[m,:,:,:,:,j])
#                 Lambda_Tens_min[l,i,m,j]=Lambda_Tens_min[l,i,m,j]+np.sum(psi_min_conj[l,:,:,:,:,i]*psi_min[m,:,:,:,:,j])
...
for omegas_m_i in omegas:
    ...
    for l in path:  #for calculating only along path in FBZ
        ...
        ###TODO: index of the momentum k+q 
        KX+kpath[l]
        ...
        Lambda_Tens_plus_kq_k=np.array([Lambda_Tens_plus[ss,:,ss,:] for ss in range(Npoi)])
        Lambda_Tens_min_kq_k=np.array([Lambda_Tens_min[ss,:,ss,:] for ss in range(Npoi)])
