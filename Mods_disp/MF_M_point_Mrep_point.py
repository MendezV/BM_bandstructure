...
class Mean_field_M:
    ...
    def __init__(self, latt, nbands, HB, symmetric, mode, cons , test, umkl):
        ...
        if self.umkl_Q-self.umkl >1:
            
            IkpMrep=np.zeros([self.latt.Npoi])
            IkpMrep=np.array(self.latt.insertion_index( self.latt.KX+Mp_rep[0],self.latt.KY+Mp_rep[1], self.latt.KQX, self.latt.KQY))
            self.IkpMrep=IkpMrep.astype(int)
            print( 'the shape of the index Mrep array',np.shape(self.IkpM))
            
            IkmMrep=np.zeros([self.latt.Npoi])
            IkmMrep=np.array(self.latt.insertion_index( self.latt.KX-Mp_rep[0],self.latt.KY-Mp_rep[1], self.latt.KQX, self.latt.KQY))
            ...
            IMF_M_kpMrep=np.array(self.latt.insertion_index( self.latt.KQX[self.IkMF_M_1bz]+Mp_rep[0],self.latt.KQY[self.IkMF_M_1bz]+Mp_rep[1], self.latt.KQX, self.latt.KQY))
            self.IMF_M_kpMrep=IMF_M_kpMrep.astype(int)
            print( 'the shape of the index Mrep array',np.shape(self.IMF_M_kpM))
            
            IMF_M_kmMrep=np.zeros([self.Npoi_MF])
            IMF_M_kmMrep=np.array(self.latt.insertion_index( self.latt.KQX[self.IkMF_M_1bz]-Mp_rep[0],self.latt.KQY[self.IkMF_M_1bz]-Mp_rep[1], self.latt.KQX, self.latt.KQY))
            self.IMF_M_kmMrep=IMF_M_kmMrep.astype(int)
            print( 'the shape of the index Mrep array',np.shape(self.IMF_M_kmM))
            
        else:
            print('Need more umklapps to accomodat the M repetition shifted Form Factors')
    ...
    def H_MF_parts_MBZ(self):
        ...
        for Nk in range(self.latt.Npoi):  #for calculating only along path in FBZ
            ...
            ikq2=self.IkpMrep[Nk]
            ikmq2=self.IkmMrep[Nk]
    ...
    def H_MF_parts_rMBZ(self):
        ...
        for Nk in range(self.Npoi_MF):  #for calculating only along path in FBZ
            ...
            ikq2=self.IMF_M_kpMrep[Nk]
            ikmq2=self.IMF_M_kmMrep[Nk]
...
def main() -> int:
    ...
    l=MoireLattice.MoireTriangLattice(Nsamp, theta, 0, c6sym, umkl, umklq = 1)
    lq=MoireLattice.MoireTriangLattice(Nsamp, theta, 2, c6sym, umkl, umklq = 1) #this one is normalized
    [q1,q2,q3]=l.q
    q=np.sqrt(q1@q1)
