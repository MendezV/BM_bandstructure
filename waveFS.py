...
def eigsystem2(kx, ky, xi, nbands, n1arg, n2arg):
    ...
    for i in range(N):
        Q1x=G0x + xi*q1[0]
        Q1y=G0y + xi*q1[1]
        Q2x=G0x - xi*q1[0] - xi*q1[0]
        Q2y=G0y - xi*q1[1] - xi*q1[1]
        # Q2xp=G0x - xi*q1[0] - q1[0]
        # Q2yp=G0y - xi*q1[1] - q1[1]
        indi1=np.where((np.abs(Q1x-Q2x[i])<1e-10)*(np.abs(Q1y-Q2y[i])<1e-10))
        # indi2=np.where((np.abs(Q1x-Q2xp[i])<1e-8)*(np.abs(Q1y-Q2yp[i])<1e-8))
        if np.size(indi1)>0:
            matGq1[i,indi1]=1
        # if np.size(indi2)>0:
        #     matGq1[i,indi2]=1
        Q2x=G0x - xi*q1[0] - xi*q2[0]
        Q2y=G0y - xi*q1[1] - xi*q2[1]
        # Q2xp=G0x - xi*q1[0] - q2[0]
        # Q2yp=G0y - xi*q1[1] - q2[1]
        # indi1=np.where((Q1x==Q2x[i])*(Q1y==Q2y[i]))
        indi1=np.where((np.abs(Q1x-Q2x[i])<1e-8)*(np.abs(Q1y-Q2y[i])<1e-8))
        # indi2=np.where((np.abs(Q1x-Q2xp[i])<1e-8)*(np.abs(Q1y-Q2yp[i])<1e-8))
        if np.size(indi1)>0:
            matGq2[i,indi1]=1
        # if np.size(indi2)>0:
        #     matGq2[i,indi2]=1
        Q2x=G0x - xi*q1[0] - xi*q3[0]
        Q2y=G0y - xi*q1[1] - xi*q3[1]
        # Q2xp=G0x - xi*q1[0] - q3[0]
        # Q2yp=G0y - xi*q1[1] - q3[1]
        # indi1=np.where((Q1x==Q2x[i])*(Q1y==Q2y[i]))
        indi1=np.where((np.abs(Q1x-Q2x[i])<1e-8)*(np.abs(Q1y-Q2y[i])<1e-8))
        # indi2=np.where((np.abs(Q1x-Q2xp[i])<1e-8)*(np.abs(Q1y-Q2yp[i])<1e-8))
        if np.size(indi1)>0:
    ...
    #assembling the matrix to be diagonalized
    U=(np.kron(matu1,matGq1)+np.kron(matu2,matGq2)+np.kron(matu3,matGq3)) #interlayer coupling
    Udag=(U.conj()).T
    H1=-hv*(np.kron(xi*paulix,np.diag(qx_1))+np.kron(pauliy,np.diag(qy_1)))#+np.kron(pauliz,gap*np.eye(N)) # ARITCFICIAL GAP ADDED
    H2=-hv*(np.kron(xi*paulix,np.diag(qx_2))+np.kron(pauliy,np.diag(qy_2)))#+np.kron(pauliz,gap*np.eye(N)) # ARITCFICIAL GAP ADDED
    Hxi=np.bmat([[H1, Udag ], [U, H2]]) #Full matrix
